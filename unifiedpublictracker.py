...
class unifiedpublictracker(object):
    ...
    def search(self, what, cat="all"):
        # qBittorrent passes the query URL-encoded (spaces -> %20). Decode once
        # here so each engine can re-encode exactly once; otherwise quote_plus
        # double-encodes (%20 -> %2520) and multi-word searches return nothing.
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(engines)) as executor:
            futures = {executor.submit(fn, what): fn for fn in engines}
            for future in concurrent.futures.as_completed(futures):
                engine_fn = futures[future]
                try:
                    ...
                    # If one engine fails, we continue with others
                    pass
    ...
    def _yts(self, what):
        ...
        for domain in ["https://yts.mx", "https://yts.am", "https://yts.lt"]:
            try:
                data = retrieve_url("{}/api/v2/list_movies.json?{}".format(domain, urlencode(params)))
                if data:
                    break
            except Exception:
                continue
    ...
    def _bitsearch(self, what):
        ...
        for page in range(1, 4):
            url = "https://bitsearch.eu/api/v1/search?q={}&page={}&sort=seeders".format(quote_plus(what), page)
            ...
            for t in torrents:
                ...
                results.append({
                    ...
                    "engine_url": "https://bitsearch.eu/",
                    "desc_link": "https://bitsearch.eu/torrent/{}".format(t.get("id", "")),
                })
        return results
    ...
    def _rutor(self, what):
        ...
        for page in range(3):
            url = "{}/search/{}/0/000/2/{}".format(base, page, quote_plus(what))
    ...
    def _nyaa(self, what):
        ...
        base = "https://nyaa.si"
        for page in range(1, 4):
            url = "{}/?f=0&c=0_0&q={}&p={}&s=seeders&o=desc".format(base, quote_plus(what), page)
            html = retrieve_url(url)
            if not html:
                break
